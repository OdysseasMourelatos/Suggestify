...
# PATH TO JAVA JAR
# ══════════════════════════════════════════════════════════════════
JAVA_JAR_PATH = r"C:\Users\spmou\Documents\ODY\Suggestify\out\artifacts\SuggestifyProject_jar\SuggestifyProject.jar"
...
def run_java_parser(zip_path: str):
    """
    Calls the Java JAR with the zip path as argument.
    Your SpotifyParser.main() should accept args[0] as the zip path
    (change the hardcoded path to: String zipFilePath = args.length > 0 ? args[0] : "default.zip";)
    """
    cmd = ["java", "-jar", JAVA_JAR_PATH, zip_path]
...
# ══════════════════════════════════════════════════════════════════
    ...
    def java_thread():
        try:
            result_holder["result"] = run_java_parser(st.session_state.saved_zip_path)
